...
from flaskr.model import Role, RoleSchema
...
def bulkUpdate():
    ...
    items = [Role.query.with_for_update().get(id) for id in ids] # 加锁
    for item in items:
        item.update(**kwds)
    db.session.commit()
    return {
...
def bulkDestroy():
    ...
    if len(ids) == 0:
        abort(400, "ID参数不能为空")
    items = [Role.query.get(id) for id in ids]
    for item in items:
        item.delete()
    db.session.commit()
    return {
...
def findByPk(id):
    ...
    item = Role.query.get_or_404_(id)
    return schema.dump(item)


def updateByPk(id):
    """更新单条"""
    item = Role.query.with_for_update().get(id) # 此处加锁
...
def bulkCreate():
    ...
    items = [Role(**kwds) for kwds in kwds_list]
