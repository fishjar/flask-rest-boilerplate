...
from flaskr.model import User, UserSchema
...
def bulkUpdate():
    ...
    items = [User.query.with_for_update().get(id) for id in ids] # 加锁
    for item in items:
        item.update(**kwds)
    db.session.commit()
    return {
...
def findByPk(id):
    ...
    item = User.query.get_or_404_(id)
    return schema.dump(item)
...
def updateByPk(id):
    """更新单条"""
    # item = User.query.get_or_404_(id)
    item = User.query.with_for_update().get(id) # 此处加锁
