...
def findAndCountAll():
    ...
    data = UserGroup.query.paginate_(
...
def bulkUpdate():
    ...
    items = [UserGroup.query.with_for_update().get(id) for id in ids] # 加锁
    for item in items:
        item.update(**kwds)
    db.session.commit()
    return {
...
def bulkDestroy():
    ...
    if len(ids) == 0:
        abort(400, "ID参数不能为空")
    items = [UserGroup.query.get(id) for id in ids]
    for item in items:
        item.delete()
    db.session.commit()
    return {
...
def findByPk(id):
    """根据主键查询单条信息"""
    # item = UserGroup.query.get(id)
    # if item is None:
    #     abort(404, description="记录不存在")
    item = UserGroup.query.get_or_404_(id)
    return schema.dump(item)


def updateByPk(id):
    """更新单条"""
    item = UserGroupquery.with_for_update().get(id) # 此处加锁
...
def bulkCreate():
    ...
    items = [UserGroup(**kwds) for kwds in kwds_list]
