...
def bulkUpdate():
    ...
    items = [Menu.query.with_for_update().get(id) for id in ids] # 加锁
    for item in items:
        item.update(**kwds)
    db.session.commit()
    return {
...
def findByPk(id):
    ...
    item = Menu.query.get_or_404_(id)
    return schema.dump(item)


def updateByPk(id):
    """更新单条"""
    item = Menu.query.with_for_update().get(id) # 此处加锁
